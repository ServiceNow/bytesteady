...
import os
import pandas as pd
import sys; sys.path.append("/mnt/home/adrouin/dev/element_ai/bytesteady/data/")
...
RAW_DATA_PATH = "/mnt/datasets/public/research/antibiotic_resistance/gene_classification/"
...
def write_dataset(name, kmer_to_idx):
    ...
    """
    dataset = pd.read_csv(os.path.join(RAW_DATA_PATH, "%s.%s" % (name, RAW_DATA_EXT)), sep="\t")
    ...
    for i, (idx, row) in enumerate(tqdm(dataset.iterrows(), total=dataset.shape[0])):
        ...
        if len(kmer_idx) == 0:
            print("\n" * 3, sequence, "\n" * 3)
            exit()
        data_line = "%d | %s\n" % (row.label + 1, " ".join(["k%d:%d" % (k, c) for k, c in zip(kmer_idx, counts)]))
