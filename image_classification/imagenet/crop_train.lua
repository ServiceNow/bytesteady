...
Usage: th crop_train.lua [category_file] [data_directory] [output_directory]
   [gravity] [size] [quality] [threads]
--]]
...
function joe.main()
   ...
   local data_directory = arg[2] or 'data/trainsize72quality75'
   local output_directory = arg[3] or 'data/trainsize72quality75centercrop64'
   local gravity = arg[4] or 'center'
   local size = arg[5] or '64'
   local quality = arg[6] or '75'
   local threads = arg[7] and tonumber(arg[7]) or 8

   joe.constructTrain(category_file, data_directory, output_directory, gravity,
                      size, quality, threads)
end

function joe.constructTrain(
      category_file, data_directory, output_directory, gravity, size, quality,
      threads)
   -- Tunnel RPC executer
