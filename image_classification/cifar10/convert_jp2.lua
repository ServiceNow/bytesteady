...
Usage: th convert_train.lua [data_directory] [output_directory] [size] [rate]
   [threads]
--]]
...
function joe.main()
   ...
   local output_directory = arg[2] or 'data/trainsize32rate1jp2'
   local size = arg[3] or '32'
   local rate = arg[4] or '1'
   local threads = arg[5] and tonumber(arg[5]) or 8

   joe.constructTrain(data_directory, output_directory, size, rate, threads)
end

function joe.constructTrain(
      data_directory, output_directory, size, rate, threads)
   -- Tunnel RPC executer
   ...
   local convert_options = '-colorspace RGB -depth 24'..
      ...
      ' -define jp2:number-resolutions=6'..
      ' -define jp2:rate='..rate
   local category = 0
      ...
      for file in paths.iterfiles(category_directory) do
         local cmd = 'convert '..paths.concat(category_directory, file)..
            ' '..convert_options..' '..paths.concat(
               convert_directory, paths.basename(file, '.jpg')..'.jp2')
