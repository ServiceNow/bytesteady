...
LOCATION=models/${DATA}/doublefnvnllgram${GRAM}size${SIZE}dimension${DIMENSION}a${A}b0alpha0lambda${LAMBDA}n0rho0split
...
_GRAM=${_GRAM//s/:}
_GRAM=${_GRAM//f/\},\{}
${BYTESTEADYBIN} -driver_location ${LOCATION} -driver_epoch_size 100 -driver_save 0 -data_file data/${DATA}/train_shuffle_split_0.hex -data_format kBytes -model_gram "{${_GRAM}}" -model_input_size ${SIZE} -model_output_size ${OUTPUT} -model_dimension ${DIMENSION} -train_a ${A} -train_b 0 -train_alpha 0 -train_lambda ${LAMBDA} -train_n 0 -train_rho 0 "${@:8}";
