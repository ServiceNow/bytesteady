...
void forwardBackwardTest() {
  ...
  value_type output_reference =
      input.clone().sub(input(target)).add(1.0).fmax(0.0).sum() - 1.0;
  ...
  grad_input_reference.sub(input(target)).add(1.0).fmax(0.0).isgreater(0.0);
  grad_input_reference[target].zero();
  grad_input_reference[target].fill(-grad_input_reference.sum());
  for (int i = 0; i < grad_input_reference.size(0); ++i) {
    EXPECT_FLOAT_EQ(grad_input_reference(i), grad_input(i));
  }
}

TEST(HingeLossTest, forwardBackwardTest) {
  forwardBackwardTest< DoubleHingeLoss >();
  forwardBackwardTest< FloatHingeLoss >();
}
